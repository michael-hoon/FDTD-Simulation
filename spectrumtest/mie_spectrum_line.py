...
for w in wvls:
    ...
    line_x1 = sim.add_energy(
        ...
        dfrq,
        nfrq,
        mp.EnergyRegion(center = mp.Vector3(x = -r), size = mp.Vector3(0, s), weight = -1.0)
    )
    ...
    line_x2 = sim.add_energy(
        ...
        dfrq,
        nfrq,
        mp.EnergyRegion(center = mp.Vector3(x = +r), size = mp.Vector3(0, s))
    )
    ...
    energy_back[w] = line_x1_energy
    energy_forw[w] = line_x2_energy

    plt.figure()
    plt.plot(freqs, line_x1_energy, label = f"Backwards")
    plt.plot(freqs, line_x2_energy, label = f"Forwards")
    plt.grid(True, which="both", ls="-")
    plt.xlabel("Frequency")
    plt.ylabel("Energy")
    plt.legend(loc="upper right")
    plt.title(f"Linetest_{w}")
    plt.savefig(f"Linetest_{w}.png")

plt.figure()
for w in energy_back:
    plt.plot(freqs, energy_back[w], label = f"{w*1000}nm")
plt.grid(True, which="both", ls="-")
plt.xlabel("Frequency")
plt.ylabel("Energy")
plt.legend(loc="upper right")
plt.title("Linetest_back")
plt.savefig(f"Linetest_back.png")

plt.figure()
for w in energy_back:
    plt.plot(freqs, energy_forw[w], label = f"{w*1000}nm")
plt.grid(True, which="both", ls="-")
plt.xlabel("Frequency")
plt.ylabel("Energy")
plt.legend(loc="upper right")
plt.title("Linetest_forward")
plt.savefig(f"Linetest_forward.png")
